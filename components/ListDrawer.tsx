...
export const ListPanel: React.FC<ListPanelProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="p-6 pb-0 bg-brand-50 border-b border-brand-100 flex-shrink-0 relative">
        {/* Mobile Close Button */}
        {onClose && (
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-2 text-brand-700 hover:bg-brand-100 rounded-full lg:hidden transition-colors"
            title="Fechar menu"
          >
        ...
        <div className="flex items-center gap-3 mb-6 pr-8">
           {user.photoURL ? (
             <img src={user.photoURL} alt="User" className="w-10 h-10 rounded-full border-2 border-brand-200" />
           ) : (
           ...
           <div className="overflow-hidden">
             <h2 className="text-sm font-bold text-gray-900 truncate">{user.displayName || 'Usuário'}</h2>
        ...
        <div className="mb-4">
          <button 
            ...
            className="flex items-center justify-between w-full px-1 py-1 mb-1 text-[10px] font-bold uppercase tracking-wider text-gray-400 hover:text-gray-600 transition-colors group"
          >
             <span>Estoque</span>
             <div className={`p-1 rounded hover:bg-black/5 transition-colors ${isPantryExpanded ? 'rotate-180' : ''}`}>
            ...
            <div className="relative group">
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all shadow-sm ${
                  isPantryActive 
                    ? 'bg-orange-50 border-orange-200 text-orange-800 ring-1 ring-orange-200' 
                    : 'bg-white border-gray-200 text-gray-700 hover:bg-orange-50 hover:border-orange-200'
                }`}
              >
                <div className={`p-2 rounded-lg ${isPantryActive ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500'}`}>
              ...
              {isPantryActive && (
                <button 
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-orange-400 hover:text-orange-600 hover:bg-orange-100 rounded-lg"
        ...
        <div className="flex gap-4">
          <button
            ...
            className={`pb-3 text-sm font-medium transition-colors border-b-2 ${tab === 'active' ? 'text-brand-700 border-brand-500' : 'text-gray-500 border-transparent hover:text-gray-700'}`}
          >
            Ativas
          </button>
          <button
            onClick={() => setTab('archived')}
            className={`pb-3 text-sm font-medium transition-colors border-b-2 ${tab === 'archived' ? 'text-brand-700 border-brand-500' : 'text-gray-500 border-transparent hover:text-gray-700'}`}
          >
            Arquivadas
          </button>
        </div>
          ...
          if (isEditing) {
            return (
              <form 
                ...
                className="p-3 rounded-xl bg-white border border-brand-300 shadow-sm flex flex-col gap-2"
              ...
              >
                <input 
                  ...
                  className="w-full px-2 py-1 rounded border border-gray-200 text-sm focus:border-brand-500 outline-none"
                />
                <div className="flex justify-end gap-2">
                  <button 
                    type="button" 
                    onClick={cancelEdit}
                    className="p-1.5 text-xs text-gray-500 bg-gray-100 rounded hover:bg-gray-200"
                  >
                    Cancelar
                  </button>
                  <button 
              ...
              <div className="flex flex-col overflow-hidden flex-1 mr-2">
                <div className="flex justify-between items-center mb-1">
                   <div className="flex items-center gap-1.5 truncate">
                      <span className={`font-medium truncate ${list.id === activeListId ? 'text-brand-900' : 'text-gray-700'}`}>{list.name}</span>
                      {isShared && (
                   ...
                   {totalItems > 0 && (
                     <span className={`text-[10px] font-bold ${percentage === 100 ? 'text-brand-600' : 'text-gray-400'}`}>
                       {percentage}%
                     </span>
                   )}
                </div>
                ...
                <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                   <div 
                     className={`h-full rounded-full transition-all duration-500 ${percentage === 100 ? 'bg-brand-500' : 'bg-brand-300'}`} 
                ...
                {isDeleting ? (
                   <div className="flex items-center animate-fade-in gap-1 mr-1" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        ...
                        className="p-1.5 text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-md"
                      >
                  ...
                  <div className="flex items-center gap-0.5 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      className="p-2 text-gray-400 hover:text-brand-600 hover:bg-brand-100 rounded-lg transition-colors"
                      title="Renomear"
                    >
      ...
      <div className="p-4 space-y-3 bg-gray-50 border-t border-gray-100 flex-shrink-0">
        {tab === 'active' && (
          isCreating ? (
            <form onSubmit={handleCreate} className="flex gap-2 animate-fade-in">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg border border-gray-300 focus:border-brand-500 focus:outline-none text-sm"
                value={newListName}
            ...
            <button
              ...
              className="w-full py-3 flex items-center justify-center gap-2 text-brand-700 font-medium bg-white border border-brand-200 rounded-xl hover:bg-brand-50 transition-colors shadow-sm"
            >
        ...
        <div className="grid grid-cols-2 gap-2">
            ...
            className="w-full py-2.5 flex items-center justify-center gap-2 text-gray-600 text-xs font-medium hover:bg-gray-100 rounded-lg transition-colors border border-gray-200"
            >
        ...
        <button
          onClick={signOut}
          className="w-full py-2.5 flex items-center justify-center gap-2 text-red-500 text-sm font-medium hover:bg-red-50 rounded-lg transition-colors"
        >
...
export const ListDrawer: React.FC<ListPanelProps & { isOpen: boolean }> = (props) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 w-72 bg-white shadow-2xl z-[70] lg:hidden transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
