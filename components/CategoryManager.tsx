      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md relative z-10 max-h-[85vh] flex flex-col animate-slide-up">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {categories.map((cat, index) => {
            ...
            if (isEditing) {
              return (
                <div key={cat.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-brand-200 dark:border-brand-700 space-y-3">
                  <input
                    type="text"
                    value={editName}
                    onChange={e => setEditName(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-brand-500 outline-none"
                    autoFocus
                  />
                  ...
                  <div className="flex justify-end gap-2 mt-2">
                     <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-sm text-gray-500 dark:text-gray-400">Cancelar</button>
            ...
            return (
              <div key={cat.id} className="flex items-center justify-between p-3 bg-white dark:bg-gray-700/30 border border-gray-100 dark:border-gray-700 rounded-xl hover:shadow-sm">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700">
          <form onSubmit={handleAdd} className="space-y-3">
            <input
              ...
              className="w-full px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-brand-500 outline-none"
            />
              ...
              <div className="flex gap-2">
                 {COLOR_PALETTES.slice(0, 5).map(p => (
                    <button
                      ...
                      className={`w-6 h-6 rounded-full border-2 ${p.bg} ${newColor === p.id ? 'border-gray-600 dark:border-white scale-110' : 'border-transparent'}`}
                    />
                 ))}
