...
export const LoginScreen: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-64 h-64 bg-blue-100 rounded-full blur-3xl opacity-50 pointer-events-none"></div>

      <div className="w-full max-w-sm flex flex-col items-center z-10 animate-slide-up">
        <div className="w-20 h-20 bg-brand-500 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-brand-200 mb-6 rotate-3">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-2 text-center">ListaInteligente</h1>
        <p className="text-gray-500 text-center mb-8 text-lg">
          Suas compras organizadas.
        </p>

        {error && (
          <div className="w-full mb-6 p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600 animate-fade-in flex flex-col gap-2">
             ...
             {error.includes('inválida') && (
                 <button 
                    ...
                    className="self-end text-xs font-bold text-red-700 underline hover:no-underline"
                 >
        ...
        {showConfig && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md animate-slide-up">
                    <h3 className="text-lg font-bold mb-2">Configuração do Firebase</h3>
                    ...
                    <textarea 
                        ...
                        className="w-full h-32 p-3 text-xs font-mono border rounded-lg bg-gray-50 mb-4 focus:border-brand-500 outline-none resize-none"
                    />
        ...
        <form onSubmit={handleEmailAuth} className="w-full space-y-4 mb-6">
          <div>
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none bg-white"
              disabled={isLoading}
            />
          </div>
          <div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-lg shadow-brand-200 transition-all active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
          >
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-3 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-70 disabled:active:scale-100"
        >
